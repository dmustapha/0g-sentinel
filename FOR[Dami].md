...
The revised design therefore has a **Sealed evidence** plane and a **Current access** plane. A current outage can no longer erase a valid historical artifact, and an old historical success can no longer make current admission look verified. Each stage carries its own scope, status, source block/transaction, observation time, and exact capability.

The same reasoning changes operator error handling. Once transaction submission has been attempted, “No lease issued” is not a safe default—even a provider that throws without returning a hash may have accepted the transaction. The operator workflow must distinguish failure before send, unknown submission outcome, finalized write with failed readback, and fully confirmed seal.

Recovery also cannot trust identity plus expected version: another authorized scanner could race the same version. Before paid work, ProofLock creates a durable, token-free journal row binding an idempotency/recovery ID to the canonical input and reserved budget. It then appends the verified Compute commitments, then the verified Storage commitments; the full chain input must be durable before Registry submission. Recovery can compare exact transaction/event/record facts with that saved intent without paying for Compute/Storage again or submitting a duplicate write. The same journal is the foundation for one-active-run-per-identity, multi-tab deduplication, concurrency limits, and spend ceilings.
