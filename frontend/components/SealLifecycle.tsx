...
export function SealLifecycle({ currentVerified = true, currentVersion, previousProofId, identityKey }: { currentVerified?: boolean; currentVersion?: string; previousProofId?: string; identityKey: string }) {
  const version = currentVersion && isPositiveUint64(currentVersion) ? `v${currentVersion}` : "Version unavailable";
  ...
  return <section className="evidence-card lifecycle-card"><span className="card-kicker">Versioned, append-preserved proof history</span><h3>Seal lifecycle</h3>
    <div className="lifecycle-rail"><div className="life-node current"><span><bdi dir="ltr">{version}</bdi></span>{(!currentVerified || version === "Version unavailable") && <StatusBadge status="UNAVAILABLE" surface="paper" />}<b>{currentVerified ? "CURRENT" : "REGISTRY SNAPSHOT · CURRENT UNAVAILABLE"}</b></div>
      ...
      {predecessor.status === "INVALID" && <div className="life-node"><span>Predecessor unavailable</span><StatusBadge status="UNAVAILABLE" surface="paper" /><b>UNVERIFIED LOCATOR</b><small>No historical locator link is available for the stored value.</small></div>}</div>
