export function TrustRoleDisclosure({ admin, guardian, validator, custodyConstraint }: Readonly<{
  ...
  return <aside className={`trust-disclosure ${distinct ? "state-warn" : "state-bad"}`}><h2>Named trust roles</h2>
    ...
    <p>{distinct ? "Deployment enforces three distinct addresses." : "Role handoff is missing or violates the three-distinct-address constraint."} <code>{custodyConstraint ?? "custody constraint not configured"}</code>. Drift detection is on-demand, not continuous.</p></aside>;
}

function Role({ name, address, note }: { name: string; address?: string; note: string }) {
  return <div><dt>{name}</dt><dd><code className="break">{address ?? "not configured"}</code><small>{note}</small></dd></div>;
}
