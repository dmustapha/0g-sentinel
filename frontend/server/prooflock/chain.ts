import { AbiCoder, Interface, getAddress, keccak256, type Provider, type Signer } from "ethers";
...
export type ChainWriteRequest = Readonly<{
  registryAddress: HexAddress; mode: "SEAL" | "RESEAL"; expectedPriorVersion?: bigint;
  identityKey: Bytes32; subject: HexAddress; envelopeDigest: Bytes32; storageRoot: Bytes32;
...
export async function writeProofLock(
  ...
  const expectedVersion = requireWriteState(request, current.version);
...
export async function readProofLockBack(
  ...
  if (write.expectedVersion !== requiredVersion) {
    throw new ChainProofError("READBACK_MISMATCH", "Write result version mismatch");
  }
  bytes32(write.transactionHash, false, "write transaction hash");
  const record = await adapter.getProofLock(request.identityKey);
  const expected = expectedRecord(request, write.expectedVersion);
...
function validateRequest(value: ChainWriteRequest): ChainWriteRequest {
  ...
  const request = {
    ...value,
    registryAddress: normalizeAddress(value.registryAddress, "registry"),
    subject: normalizeAddress(value.subject, "subject"),
    identityKey: bytes32(value.identityKey, false, "identity key"),
    envelopeDigest: bytes32(value.envelopeDigest, false, "envelope digest"),
    storageRoot: bytes32(value.storageRoot, false, "storage root"),
    computeRoot: bytes32(value.computeRoot, false, "compute root"),
    artifactHash: bytes32(value.artifactHash, false, "artifact hash"),
    runtimeCodeHash: bytes32(value.runtimeCodeHash, true, "runtime code hash"),
  };
  ...
  if (request.mode === "RESEAL" && (!request.expectedPriorVersion || request.expectedPriorVersion < 1n)) {
...
function validateOptions(value: Readonly<{ confirmations: number; timeoutMs: number }>) {
  integer(value.confirmations, 1, MAX_CONFIRMATIONS, "confirmations");
  integer(value.timeoutMs, 1, MAX_TIMEOUT_MS, "receipt timeout");
  return value;
}
...
function requireWriteState(request: ChainWriteRequest, version: bigint): bigint {
  if (request.mode === "SEAL" && version !== 0n) {
    throw new ChainProofError("LOCK_STATE_MISMATCH", "Seal requires an absent ProofLock");
  }
  if (request.mode === "RESEAL" && version !== request.expectedPriorVersion) {
    throw new ChainProofError("LOCK_STATE_MISMATCH", "Reseal prior version mismatch");
  }
  return request.mode === "SEAL" ? 1n : version + 1n;
}
...
async function finalize(adapter: RegistryChainAdapter, hash: string, options: { confirmations: number; timeoutMs: number }) {
  ...
  if (receipt.confirmations < options.confirmations) {
...
function assertLockEvent(receipt: RegistryReceipt, request: ChainWriteRequest, version: bigint): void {
  const matches = receipt.logs.filter((log) => normalizeAddress(log.address, "event registry") === request.registryAddress)
    .map((log) => { try { return REGISTRY_V2_INTERFACE.parseLog({ topics: [...log.topics], data: log.data }); } catch { return null; } })
    .filter((event) => event?.name === "ProofLocked");
